...
export default function Login() {
  ...
  return <main className="auth-page"><div className="auth-card"><img src={logo} className="auth-logo" alt="Goo Amrutham"/><span className="eyebrow">WELCOME BACK</span><h1>Sign in</h1><p>Access your orders, live tracking and profile.</p>
    ...
    <form onSubmit={submit}><label>Email<input type="email" className="form-control" required value={form.email} onChange={e=>setForm({...form,email:e.target.value})}/></label><label>Password<input type="password" className="form-control" required value={form.password} onChange={e=>setForm({...form,password:e.target.value})}/></label><button disabled={busy || !supabaseConfigured} className="btn btn-success w-100 btn-lg rounded-pill">{busy ? "Signing in…" : "Sign In"}</button></form>
